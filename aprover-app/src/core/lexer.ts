...
export class Lexer {
  ...
  private readCharLit(): string {
    ...
    this.advance();
    if (this.current() !== "'") {
      throw new LexerError('Expected closing quote in character literal', this.line, this.column, this.pos);
    }
  ...
  nextToken(): Token {
    ...
    switch (c) {
      case ':':
        this.advance();
        return { type: 'DEFINE', value: ':', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case '=':
        this.advance();
      ...
      case '♀':
        this.advance();
        return { type: 'FEMALE', value: '♀', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case '!':
        this.advance();
      ...
      case '∞':
        this.advance();
        return { type: 'INFINITY', value: '∞', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case '(':
        this.advance();
        return { type: 'LPAREN', value: '(', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case ')':
        this.advance();
        return { type: 'RPAREN', value: ')', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case '{':
        this.advance();
        return { type: 'LBRACE', value: '{', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case '}':
        this.advance();
        return { type: 'RBRACE', value: '}', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case '[':
        this.advance();
        return { type: 'LBRACKET', value: '[', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case ']':
        this.advance();
        return { type: 'RBRACKET', value: ']', loc: this.makeLoc(startLine, startColumn, startOffset) };
      case ',':
        this.advance();
      ...
      case "'":
        ...
        return { type: 'CHAR_LIT', value: char, loc: this.makeLoc(startLine, startColumn, startOffset) };
    }
